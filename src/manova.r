...
#prepare manova tests
summary(skulls.manova1, test = "Hotelling-Lawley") #sees if one mean is different
...
skulls.manova2 = manova(cbind(mb, bh, bl, nh) ~ as.factor(epoch), data = skulls,
                        subset = as.factor(epoch) %in% c("c4000BC", "c200BC"))

summary(skulls.manova2) #shows that there is difference between all skulls in C4000BC and C200BC
